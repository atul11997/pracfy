...
@endif
        ...
        <div class="card shadow-sm border-0">
            <div class="card-header bg-secodary text-primary">
                <div class="d-flex justify-content-between w-100">
                    ...
                    <a data-bs-toggle="modal" data-bs-target="#testimonials" class="btn btn-primary"><i
                            class="bi bi-plus"></i>
        ...
        {{-- Google Review to google Account --}}
        <div class="card shadow-sm border-0">
            <div class="card-header bg-secodary text-primary">
                <div class="d-flex justify-content-between align-items-center w-100">
                    <h3 class="mb-0">Testimonial Direct Google Review List</h3>
                        <a href="{{ route('testimonials.google.redirect') }}" class="btn btn-primary">
                            <i class="bi bi-plus"></i> Connect your google
                        </a>
                </div>
            </div>
            <div class="card-body">
    ...
    {{-- Edit Testimonial --}}
    <div class="modal fade" id="edittestimonials" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
...
$(document).ready(function () {
    $.ajax({
        url: '{{ route("testimonials.get.google.reviews") }}',
        type: 'GET',
        success: function (response) {
            ...
            if (response.reviews && response.reviews.length > 0) {
                ...
                response.reviews.forEach((review, index) => {
                    html += `
                        <tr>
                            <td>${index + 1}</td>
                            <td>${review.name}</td>
                            <td><img src="${review.profile_image}" width="50" class="rounded-circle" /></td>
                            <td>${review.rating} ⭐</td>
                ...
                $('#google-review').html(html);
            } else {
                $('#google-review').html('<tr><td colspan="6" class="text-center">No reviews found.</td></tr>');
            }
        },
        error: function (xhr) {
            $('#google-review').html('<tr><td colspan="6" class="text-center text-danger">Error loading reviews.</td></tr>');
        }
    });
});
    </script>
