                    ...
                    @foreach ($photos as $key => $photo)
                        <div class="col-6 col-sm-4 col-md-3 col-lg-2 position-relative">
                            <div class="border rounded shadow-sm overflow-hidden">
                                @if($photo->photos_upload || file_exists($photo->photos_upload))
                                <img src="{{ $photo->photos_upload ?? '' }}" alt="Photo {{ $key + 1 }}"
                                    class="img-fluid w-100" style="height: 150px; object-fit: cover;">
                                @else
                                <img src="{{ url('/') }}/assets/img/No_Image_Available.jpg" alt="Photo"
                                    class="img-fluid w-100" style="height: 150px; object-fit: cover;">
                                @endif
        ...
        $('#galleryForm').on('submit', function(e) {
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    ...
                    $('#ajaxerrmsg').html(
                        `<div class="alert alert-danger d-flex align-items-center" role="alert">
        <i class="bi bi-x-circle-fill me-2"></i><div>Something went wrong. Please try again.</div></div>`
                    );
                }
            });
        });
