...
// });
    ...
    Route::get('/terms-of-services', [SiteController::class, 'termsServices'])->name('terms.services');
  ...
  //Admin Panel Routs
    ...
    //Page
    Route::prefix('admin')->name('section.')->group(function () {
        Route::get('section-list', [PageController::class, 'pageList'])->name('list');
        Route::post('section-process', [PageController::class, 'pageProcess'])->name('process');
        Route::get('section-edit/{id}', [PageController::class, 'editPage'])->name('edit');
        Route::post('section-update', [PageController::class, 'updatePage'])->name('update');
        Route::get('section-status-change/{id}/{status}', [PageController::class, 'statusChangePage'])->name('status.change');
    });
    ...
    Route::prefix('admin')->name('video.')->group(function () {
        ...
        Route::post('/video/upload-temp', [VideoController::class, 'uploadTemp'])->name('upload.temp');
    });
    ...
    Route::prefix('admin')->name('testimonials.')->group(function () {
        ...
        Route::get('/google/redirect', [TestimonialController::class, 'redirectGoogle'])->name('google.redirect');
        Route::get('/google/callback', [TestimonialController::class, 'handleGoogleCallback'])->name('google.callback');
        Route::get('/get-google-reviews', [TestimonialController::class, 'fetchGoogleReviews'])->name('get.google.reviews');
    });
    ...
    Route::get('/admin/theme-settings', [ThemeController::class, 'index'])->name('theme.setting');
    Route::post('/admin/theme-settings', [ThemeController::class, 'update'])->name('theme.setting.update');
    ...
    Route::get('/get/theme/color', [SiteController::class, 'get_theme_customization'])->name('get.theme.color');

    Route::get('/blog-details/{blogid}', [SiteController::class, 'blogDetails'])->name('blog.details');
});
Route::get('/theme.css', function () {
    $css = "
:root{
    --primary:    " . setting('theme.primary_color', '#3498db') . ";
    --bg:         " . setting('theme.background_color', '#ffffff') . ";
    --color:      " . setting('theme.color', '#2980b9') . ";
    --hover:      " . setting('theme.hover_color', '#2980b9') . ";
    --active:     " . setting('theme.active_color', '#1f6fb2') . ";
    --font:       '" . setting('theme.font_family', 'Arial') . "', sans-serif;
    --font-size:  " . setting('theme.font_size', '16px') . ";
    --font-wt:    " . setting('theme.font_weight', '400') . ";
}
body{ color: var(--color); background: var(--bg); font-family: var(--font); font-size: var(--font-size); font-weight: var(--font-wt); }
