...
class AuthController extends Controller
{
    ...
    public function registerProcess(Request $request)
    {
        ...
        $data = $request->only(['name', 'email', 'city']);
        ...
        if(User::where('email', $data['email'])->exists()){
            return back()->with('error', 'Doctor Already Created Account');
        }
    ...
    public function dashboard(){
        $totalsections = Page::where('user_id', Auth::user()->id)->where('status', 0)->count();
        $totalservices = Service::where('user_id', Auth::user()->id)->count();
        $totalphotos = Gallery::where('user_id', Auth::user()->id)->count();
        $totalvideo = Video::where('user_id', Auth::user()->id)->count();
        $totalfaq = Faq::where('user_id', Auth::user()->id)->count();
        $totalblog = Blog::where('userid', Auth::user()->id)->count();
        $totaldoctor = Doctor::where('user_id', Auth::user()->id)->count();
        $totaldepartments = Department::where('user_id', Auth::user()->id)->count();
        return view('dashboard', compact('totalsections', 'totalservices', 'totalphotos', 'totalphotos', 'totalvideo', 'totalfaq', 'totalblog', 'totaldepartments', 'totaldoctor'));
    }
